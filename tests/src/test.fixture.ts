...
const PLUGIN_LIB_NANOX = { 'Lens': Resolve("elfs/plugin_nanox.elf") };
const PLUGIN_LIB_NANOSP = { 'Lens': Resolve("elfs/plugin_nanosp.elf") };
...
async function processTransaction(eth, sim, steps, label, rawTxHex, srlTx = "") {
  ...
  const resolution = await ledgerService
    ...
    .catch((e) => {
      console.warn("an error occurred in resolveTransaction => fallback to blind signing: " + e.stack);
      return null;
    });
